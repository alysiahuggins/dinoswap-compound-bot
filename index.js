...
const SUSHI_SWAP="0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506"

const SUSHI_SWAP_LP_CONTRACT = "0x3324af8417844e70b81555A6D1568d78f4D4Bf1f"
...
//10 dino-usdc :   12 - ust-usdc   18 - dnxc
...
async function checkCompoundingOpportunities(){
    ...
    try{
    	...
    	let dinoFarmContract = new web3.eth.Contract(DINO_FARM_ABI, DINO_FARM_CONTRACT)
        ...
        if(pendingDinoAmount > 0) {
            ...
            let swapAmount = sumPendingDino * .75
            //compound(sumPendingDino.toString(), gasPrice, gasLimit)
            await swap('100000000000000000', gasPrice, gasLimit)
            await add_liquidty('45000000000000000', gasPrice, 250000)
            await deposit_dino(10, gasPrice, gasLimit)
        }
        else{
...
async function deposit_dino(dinoFarmId, gasPrice, gasLimit){

	let dinoFarmContract = new web3.eth.Contract(DINO_FARM_ABI, DINO_FARM_CONTRACT)

	let lpContract = new web3.eth.Contract(SUSHI_SWAP_LP_ABI, SUSHI_SWAP_LP_CONTRACT)
    let balance = await lpContract.methods.balanceOf(wallet.address).call();
	console.log(`lpBalance ${balance}`)



  	let x = await dinoFarmContract.methods.deposit(dinoFarmId, balance).send(
            {
            	from: wallet.address,
            	gas: gasLimit,
            	gasPrice: gasPrice
	...
	console.log(`deposited dino ${x.status}`)
}


/*
0	tokenA	address	2791bca1f2de4661ed88a30c99a7a9449aa84174
1	tokenB	address	aa9654becca45b5bdfa5ac646c939c62b527d394
2	amountADesired	uint256	81396
3	amountBDesired	uint256	98333875960532204
4	amountAMin	uint256	81315
5	amountBMin	uint256	98235542084571671
6	to	address	21549754eda4fa4f631b07ed52576cb8a8c4940e
7	deadline	uint256	1630181076


*/
async function add_liquidty(pendingDino, gasPrice, gasLimit){

	let sushiContract = new web3.eth.Contract(SUSHI_ABI, SUSHI_SWAP)

	tokenA = "0x2791bca1f2de4661ed88a30c99a7a9449aa84174" //usdc token
	tokenB = "0xaa9654becca45b5bdfa5ac646c939c62b527d394" // dino token
	amountADesired = 2 //basically don't care about the price
	amountBDesired = pendingDino
	amountAMin = 1
	amountBMin = 1
	to = wallet.address
	deadline = 9630177423

	let abc = sushiContract.methods.addLiquidity(tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin, to, deadline).send(
            {
            	from: wallet.address,
            	gas: gasLimit,
            	gasPrice: gasPrice
    ...
    await abc
    console.log('done with the add liq')


}


async function swap(pendingDino, gasPrice, gasLimit){
	let sushiContract = new web3.eth.Contract(SUSHI_ABI, SUSHI_SWAP)
	amountIn = pendingDino
	amountOutMin = 1
	//path = [ new BN("aa9654becca45b5bdfa5ac646c939c62b527d394"), new BN("2791bca1f2de4661ed88a30c99a7a9449aa84174")]
	path = [ "0xaa9654becca45b5bdfa5ac646c939c62b527d394", "0x2791bca1f2de4661ed88a30c99a7a9449aa84174"]
	to  = '21549754eda4fa4f631b07ed52576cb8a8c4940e'
	deadline = 9630177423

	let abc = sushiContract.methods.swapExactTokensForTokens(amountIn, amountOutMin, path, wallet.address, deadline).send(
            {
            	from: wallet.address,
            	gas: gasLimit,
            	gasPrice: gasPrice
    ...
    await abc
    console.log('done with the swap')
}


async function compound(pendingDino, gasPrice, gasLimit){
    ...
    try {
    	...
    	console.log(`DINO withdraw error ${err}`)
    ...
    try{
    	...
    	let poolContract = new web3.eth.Contract(SFI_POOL_ABI, UST_POOL_CONTRACT)
...
setInterval(checkCompoundingOpportunities, POLLING_INTERVAL)
